...
- The url of the 'Next', 'More', 'See more', 'load more' or any other button indicating how to access the next page, if any: It should be one url and no more, if there are multiple urls with the same structure, discard this url from output.
...
- A list of page URLs for pagination: It should be a pattern of similar urls with pages that are numbered, if you detect this pattern and the numbers starts from a certain low number until a large number generate the rest of the urls even if they're not included.
  Your goal here is to give as many urls for the user to choose from in order for them to do further scraping, you will have to deal with very different websites that can potentially have so many urls of images and other elements.
